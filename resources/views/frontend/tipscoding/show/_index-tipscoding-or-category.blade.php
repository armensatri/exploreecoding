<div
  class="flex flex-col items-center justify-center mb-10 gap-5 md:flex-row">
  <a href="{{ route('ec-tipscodings.index') }}">
    <button
      class="flex items-center cursor-pointer pl-3 pr-1.5 py-1.5 rounded-[10px] border text-[15px] font-medium text-blue-600 border-blue-400 bg-gray-100 tracking-wide hover:text-black hover:bg-gray-200 whitespace-nowrap">
      🔹
      <img src="{{ asset(
        'frontend/img/navigation/tipscodings.png') }}" alt="image"
        class="object-contain w-4 h-4 mr-2"
      />

      <span
        class="mr-1 text-green-600 underline underline-offset-2 decoration-green-500">
        Semua tipscoding
      </span>

      <span
        class="px-1.5 py-0.5 text-gray-800 bg-gray-100 rounded-md text-[11px] ml-1 border border-gray-400 tracking-wider">
        {{ $tipstotal }} Tips
      </span>
    </button>
  </a>

  <a href="{{ route('ec-categories.index') }}">
    <button
      class="flex items-center cursor-pointer pl-3 pr-1.5 py-1.5 rounded-[10px] border text-[15px] font-medium text-blue-600 border-blue-400 bg-gray-100 tracking-wide hover:text-black hover:bg-gray-200 whitespace-nowrap">
      🔹
      <img src="{{ asset(
        'frontend/img/random/categories.png') }}"
        alt="image"
        class="object-contain w-4 h-4 mr-2"
      />

      <span
        class="mr-1 text-green-600 underline underline-offset-2 decoration-green-500">
        Semua categories
      </span>

      <span
        class="px-1.5 py-0.5 text-gray-800 bg-gray-100 rounded-md text-[11px] ml-1 border border-gray-400 tracking-wider">
        {{ $categorytotal }} Ctg
      </span>
    </button>
  </a>
</div>
